...
fs.readFile('src/index.html', 'utf8', (readError, markup) => {
  ...
  if (useTrackJs) {
    if (trackJsToken) {
      const trackJsCode = `
      <!-- BEGIN TRACKJS Note: This should be the first <script> on the page per https://my.trackjs.com/install -->
        <script>window._trackJs = { token: '${trackJsToken}' };</script>
      ...
      console.log(chalkWarning('To track JavaScript errors, sign up for a free trial at TrackJS.com'));
      // and enter your token in /tools/build.html on line 10.'
    }
  }
